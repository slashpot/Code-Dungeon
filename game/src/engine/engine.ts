...
// 行為變更需先過 test/engine.test.ts 的對拍案例。
//
...
export interface EnemyDef { type: string; hp: number; atk: number }
export const ENEMY_DEF: Record<string, EnemyDef> = {
...
export const STOP: { readonly __stop: true } = { __stop: true }; // 內部訊號：中止玩家程式（非錯誤）

export type Dir = 'up' | 'down' | 'left' | 'right';
const DIRVEC: { [k: string]: readonly [number, number] } = {
  up: [0, -1], down: [0, 1], left: [-1, 0], right: [1, 0],
};

export interface Pos { x: number; y: number }
export interface Hero extends Pos { hp: number; potions: number }
export interface Enemy extends Pos { id: number; hp: number; maxhp: number; atk: number; type: string; ch: string }
export interface Item extends Pos { type: string }
export interface EnemySnap extends Pos { id: number; hp: number; type: string }
...
export interface GameState {
  ...
  items: Item[];
  stairs: Pos;
  seen: Record<string, boolean>;   // posKey -> true（勇者看過的格子，API 用）
  dist: Record<string, number>;    // posKey -> 與勇者的 BFS 距離（reveal 更新）
  turn: number;
  kills: number;
  dmgTaken: number;
  lastHit: string | null;
...
export type GameStatus = 'playing' | 'won' | 'dead' | 'timeout';
export type LogClass = '' | 'good' | 'bad' | 'warn' | 'sys';
export type LogFn = (msg: string, cls: LogClass, turn: number) => void;

// 玩家 API v0（14 函式＋log）。行動類每呼叫一次＝一回合。
export interface GameApi {
  alive(): boolean;
  hp(): number;
  hasPotion(): boolean;
  getEnemies(): EnemySnap[];
  getItems(): Item[];
  getStairs(): Pos | null;
  myPos(): Pos;
  distance(t: Pos): number;
  nearest<T>(list: T[] | null | undefined): T | null;
  move(dir: string): void;
  moveToward(t: Pos | null | undefined): void;
  attack(t: { id: number } | null | undefined): void;
  drinkPotion(): void;
  explore(dir?: string | null): void;
  log(msg: unknown): void;
}

export type BfsGoal = (x: number, y: number) => boolean;
export type BfsBlocked = (x: number, y: number) => boolean;
...
export interface Game {
  ...
  isPoison(x: number, y: number): boolean;
  visibleNow(x: number, y: number): boolean;
  bfs(sx: number, sy: number, goal: BfsGoal, blocked?: BfsBlocked | null, dirOrder?: ReadonlyArray<readonly [number, number]>): number[][] | null;
}

export interface CreateGameOpts {
  onLog?: LogFn;
  onTick?: (s: GameState) => void; // 對拍用：turn++ 後、世界反應前（與 prototype 測試的 patch 點一致）
  turnCap?: number;
}

export function createGame(level: Level, opts: CreateGameOpts = {}): Game {
  const MAPSRC = level.map;
  const H = MAPSRC.length;
  const W = MAPSRC[0].length;
  const onLog: LogFn = opts.onLog || function () {};
  ...
  const state: GameState = {
    ...
    enemies: [],
    items: [],
    stairs: { x: -1, y: -1 }, // 初始化後掃描地圖填入（每張圖都有 >）
    seen: {},
    dist: {},
  ...
  function isWall(x: number, y: number): boolean { return x < 0 || y < 0 || x >= W || y >= H || MAPSRC[y][x] === "#"; }
  function isPoison(x: number, y: number): boolean { return MAPSRC[y][x] === "~"; }
  function manhattan(ax: number, ay: number, bx: number, by: number): number { return Math.abs(ax - bx) + Math.abs(ay - by); }
  ...
  function enemyBlocked(x: number, y: number): boolean { return !!enemyAt(x, y); }

  // 產生 BFS 鄰居展開順序：偏好 dir，最後才是其反方向（等距目標的方向性 tie-break）
  function dirsPreferring(dir: string | null | undefined): ReadonlyArray<readonly [number, number]> {
    const base: ReadonlyArray<readonly [number, number]> = [[0, -1], [0, 1], [-1, 0], [1, 0]];
    const pref = dir ? DIRVEC[dir] : undefined;
    if (!pref) return base;
    const opp: readonly [number, number] = [-pref[0], -pref[1]];
    const perp: Array<readonly [number, number]> = [];
    ...
    return [pref, ...perp, opp];
  }
  // 由起點朝目標的「主要方向」（曼哈頓上較大的軸），explore 看見樓梯後的預設傾向
  function dominantDir(fromX: number, fromY: number, toX: number, toY: number): Dir | null {
  ...
  function bfs(sx: number, sy: number, goal: BfsGoal, blocked?: BfsBlocked | null, dirOrder?: ReadonlyArray<readonly [number, number]>): number[][] | null {
  ...
  function reveal(): void {
    ...
    dist[posKey(state.hero.x, state.hero.y)] = 0;
    const q: Array<[number, number]> = [[state.hero.x, state.hero.y]];
    while (q.length) {
      const c = q.shift()!;
      const d = dist[posKey(c[0], c[1])];
  ...
  function visibleNow(x: number, y: number): boolean {
    const d = state.dist[posKey(x, y)];
    return d !== undefined && d <= SIGHT;
  }

  /* ---------- 回合推進 ---------- */
  // 勇者走一步：撞牆/撞敵人 = 浪費回合；走到道具上自動撿取。
  function heroStep(nx: number, ny: number): void {
    if (isWall(nx, ny)) { onLog("撞牆了，浪費一回合", "warn", state.turn); return; }
    const blocker = enemyAt(nx, ny);
    if (blocker) { onLog("被 " + blocker.type + " 擋住了", "warn", state.turn); return; }
  ...
  function guardPlaying(): void { if (status !== "playing") throw STOP; }

  /* ---------- 玩家 API ---------- */
  const snap = function (e: Enemy): EnemySnap { return { id: e.id, x: e.x, y: e.y, hp: e.hp, type: e.type }; };
  const api: GameApi = {
    ...
    moveToward: function (t) {
      ...
      const goal: BfsGoal = function (x, y) { return x === t.x && y === t.y; };
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    const c = MAPSRC[y][x];
    const def = ENEMY_DEF[c];
    if (c === "h") { state.hero.x = x; state.hero.y = y; }
    else if (def) { state.enemies.push({ id: id++, x: x, y: y, hp: def.hp, maxhp: def.hp, atk: def.atk, type: def.type, ch: c }); }
