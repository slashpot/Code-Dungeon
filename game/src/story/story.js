...
// flags 進 localStorage（`save.js`），對應驗收「新開存檔→對話→dungeon→交差流程無死路；重開能接續」。
...
export function createStory({ dialogue, onQuestChange, onMsg, onTbc }) {
  const s = loadSave().story || {};
  const flags = { intro: !!s.intro, accepted: !!s.accepted, done: !!s.done, reported: !!s.reported };
  ...
  function canEnterDungeon() { return flags.accepted; }
