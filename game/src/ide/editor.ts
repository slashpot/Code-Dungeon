// 遊戲內 IDE：CodeMirror 6（JS 高亮＋vim＋執行行反白＋vim 相對行號）。
...
export interface EditorHandle {
  ...
  setExecHighlight(line: number | null): void;
  vimEnabled(): boolean;
...
export function createEditor({ parent, doc = '', vimEnabled = true }: CreateEditorOpts): EditorHandle {
  ...
  const relRefresher = EditorView.updateListener.of((u: ViewUpdate) => {
  ...
  return {
    ...
    helpers: {
      ...
      vimInsertMode: () => {
        const cm = getCM(view) as { state?: { vim?: { insertMode?: boolean } } } | null;
        return !!(cm && cm.state && cm.state.vim && cm.state.vim.insertMode);
      },
    },
  };
}
