...
const ENEMY_RADIUS = { slime: 0.28, goblin: 0.32, 'orc-boss': 0.42 };
...
export class DungeonScene {
  constructor({ viewW, viewH }) {
    ...
    this.game = null;
    this.disp = new Map();   // 'hero' | enemyId → 顯示用像素座標 {x,y}
    this.flashUntil = new Map(); // enemyId → timestamp（被攻擊的白閃）
  }
  ...
  sync() {
    const { W, H, state } = this.game;
    const fog = this.fogLayer;
    fog.clear();
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        if (!this.game.visibleNow(x, y)) fog.rect(x * TILE, y * TILE, TILE, TILE).fill({ color: 0x05070c, alpha: 0.55 });
      }
    }
    const alive = new Set(['hero']);
  ...
  onAction(name, args) {
    if (name === 'attack' && args[0] && args[0].id !== undefined) {
      this.flashUntil.set(args[0].id, performance.now() + 160);
    }
  }
  ...
  update(dtMs) {
    ...
    const move = (key, tx, ty) => {
      const d = this.disp.get(key);
      if (!d) return { x: tx, y: ty };
      d.x += (tx - d.x) * k;
      d.y += (ty - d.y) * k;
