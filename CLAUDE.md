...
- [x] **v0.1-web M1＋M2 完成**（2026-07-31，`game/`）：M1 城市可行走（Pixi 佔位美術、逐格移動、鏡頭跟隨、互動雛形＋機房入口）；M2 一關 dungeon——grid 引擎抽成純 ES module（`game/src/engine/`，node 可直接 import，不再 sloppy-eval）、API v0 14 函式接上 Worker 直譯器管線（補了 pseudo↔native 物件轉換，spike 只驗過 primitive）、CodeMirror IDE panel（執行/全速/停止/重置/還原/vim、逐行反白、par ⭐、死亡重來、通關回城市）、**對拍全綠**：naive/es6/stock@L1、smart@L2、pro@L4 與 prototype 引擎逐回合狀態（位置/HP/藥水/敵人）完全一致。範圍護欄守住：只有 L1、無 dojo/skill/多關。
- [x] **v0.1-web M3 完成**（2026-08-01，`game/`）：基礎故事一條線——開場獨白 → 老周（`C`）接案解鎖機房門 → 通關（job done）→ 交差拿 ¤2,000＋主線鉤子（守衛記錄有 Nakamura 以外的簽名）→「To be continued」。自製打字機對話框（`src/story/dialogue.js`）、劇情 flags（intro/accepted/done/reported，`src/story/story.js`）、任務標籤、localStorage 存檔統一進 `src/save.js`（code/vim/story/pos，pos 每步落檔）——重載可接續。世界觀錨點：戰鬥都在數位層，玩家寫腳本操控 avatar，人不進機房（對齊正式版計劃 3.1「主角現實中是普通人」）。IDE 編輯器加 vim 相對行號（vim ON 時 number+relativenumber 混合）。驗收：瀏覽器九項含完整劇情流程與重載接續。下一步＝M4 收尾＋itch.io 上架。
